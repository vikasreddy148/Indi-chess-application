...
export default function OAuthCallbackPage() {
  ...
  if (error) {
    ...
    return (
      <div className="min-h-screen bg-[#fafafa] flex flex-col items-center justify-center gap-6 p-6">
        <Logo compact />
        <p className="text-slate-600 text-center max-w-sm">{message}</p>
        <Link to="/login">
          <Button variant="primary" size="lg">Back to login</Button>
        </Link>
      </div>
    )
  }
